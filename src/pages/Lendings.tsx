    ...
    <DashboardLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="stat-card-success"><span className="text-xs text-muted-foreground">{t("totalLent")}</span><p className="text-lg font-bold text-success">{fmt(totalLent)}</p></div>
          <div className="stat-card-destructive"><span className="text-xs text-muted-foreground">{t("totalBorrowed")}</span><p className="text-lg font-bold text-destructive">{fmt(totalBorrowed)}</p></div>
          <div className="stat-card"><span className="text-xs text-muted-foreground">{t("netPosition")}</span><p className={`text-lg font-bold ${totalLent - totalBorrowed >= 0 ? "text-success" : "text-destructive"}`}>{fmt(totalLent - totalBorrowed)}</p></div>
        </div>
        ...
        {lendings.length === 0 ? (
          <Card><CardContent className="py-12 text-center"><Handshake className="h-10 w-10 mx-auto mb-3 text-muted-foreground/40 animate-pulse-soft" /><p className="text-sm text-muted-foreground">{t("noLendings")}</p></CardContent></Card>
        ) : (
          <div className="space-y-3">
            {lendings.map(lending => {
              const outstanding = parseFloat(lending.amount) - parseFloat(lending.amountRepaid);
              return (
                <Card key={lending.id} className="transition-all duration-300 hover:shadow-md">
